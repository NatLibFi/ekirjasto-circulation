...
from api.controller.jwks import JwksController  # Adjust the import based on your project structure
...
class TestJWKSController:
    ...
    def setup_env(self, jwks_files_fixture: JWKSFilesFixture):
        ...
        self.jwks_file_path = os.path.join(jwks_files_fixture.directory, "r.cantook.com-jwks.json")
        with patch.dict(os.environ, {"PALACE_DEMARQUE_WEBREADER_JWK_FILE": self.jwks_file_path}):
            self.jwks_controller = JwksController()

    def test_get_jwks_return_cached_data(self, jwks_files_fixture: JWKSFilesFixture, flask_app_fixture: FlaskAppFixture):
        """Test if JWKS returns cached data correctly."""
        expected_content = jwks_files_fixture.sample_data("r.cantook.com-jwks.json")  # Load expected data from the fixture

        # Simulate cached data
        self.jwks_controller._jwks_cache['data'] = expected_content
        self.jwks_controller._jwks_cache['timestamp'] = time.time() - 1000  # Valid cache, older than TTL
    ...
    def test_get_jwks_loads_from_file(self, jwks_files_fixture: JWKSFilesFixture, flask_app_fixture: FlaskAppFixture):
        """Test if JWKS loads correctly from a file."""
        mock_jwks_data = jwks_files_fixture.sample_data("r.cantook.com-jwks.json")  # Use sample JWKS data
        ...
        with flask_app_fixture.test_request_context():
            ...
            assert self.jwks_controller._jwks_cache['timestamp'] > 0  # Ensure timestamp is set
    ...
    def test_get_jwks_file_not_found_raises_problem_detail(self, flask_app_fixture: FlaskAppFixture):
        ...
        assert problem_detail.detail == "The JWKS file could not be found on the server."
