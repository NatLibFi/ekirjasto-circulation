...
from core.opds2_import import OPDS2API, OPDS2Importer
...
def opds2_importer_fixture(
    ...
    data.importer = OPDS2Importer(db.session, data.collection)
    return data
...
class TestOPDS2Importer(OPDS2Test):
    ...
    def test_opds2_importer_correctly_imports_valid_opds2_feed(
        ...
        content_server_feed_text = opds2_files_fixture.sample_text("opds2_feed.json")
    ...
    def test_opds2_importer_skips_publications_with_unsupported_identifier_types(
        ...
        content_server_feed = opds2_files_fixture.sample_text("opds2_feed.json")
    ...
    def test_extract_next_links(
        ...
        # No next links
        assert (
            extract_next_links(opds2_files_fixture.sample_data("opds2_feed.json")) == []
        )

        # One next link
        assert extract_next_links(
            opds2_files_fixture.sample_data("opds2_feed2.json")
        ) == [
    ...
    def test_extract_last_update_dates(
        ...
        assert extract_last_update_dates(
            opds2_files_fixture.sample_data("opds2_feed.json")
        ) == [
            (
    ...
    def test_opds2_importer_imports_feeds_with_availability_info(
        ...
        feed_json = json.loads(opds2_files_fixture.sample_text("opds2_feed.json"))
        ...
        assert isinstance(imported_editions, list)
        assert len(imported_editions) == 5
